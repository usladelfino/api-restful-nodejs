...
const db = knex({
  client: 'pg',
  connection: {
    host: process.env.DB_HOST || secrets.DB_PORT,
    port: process.env.DB_PORT || secrets.DB_PORT,
    user: process.env.DB_USER || secrets.DB_USER,
    password: process.env.DB_PASSWORD || secrets.DB_PASSWORD,
    database: process.env.DB_DATABASE || secrets.DB_DATABASE
  }
});

console.log('The value of the DB_PASSWORD secret is:', process.env.DB_PASSWORD || secrets.DB_PASSWORD);
